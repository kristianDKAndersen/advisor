...
5. **Write the brief, then summon.**
   ...
   **User corrections:** Also read `~/.advisor/vault/.cache/corrections.jsonl` when it exists - these are staged corrections a user directed at the Advisor itself, a different signal from an evaluator scoring a worker. Triage them yourself; they are not lessons and carry no `task_type`. Only promote one to a real lesson via `/extract-lesson` after the same correction recurs 2+ times, matching the existing 2-failure rule - one occurrence is noise. Never paste raw correction text into a worker brief; that would inject unvetted user text into worker context. The file is often absent or empty, and that is normal, not an error.
...
- **Destructive-CLI probe (CRITICAL).** Never probe an unknown or repo-local CLI with a destructive-sounding subcommand (`delete`, `prune`, `rebuild`, `purge`, `clean`, `reset`, `migrate`) plus ANY flag, including `--help` — one such probe hard-deleted 286 vault notes. Safe exploration order: (1) bare binary invocation for usage text, (2) grep the dispatch source to understand subcommand routing, (3) pair with `--dry-run` before executing. This rule does NOT apply to well-known system tools (`git`, `npm`, `gh`, `jq`). `bin/advisor-audit-clis` is the standing audit that enforces the help-before-side-effect invariant across every subcommand-dispatching script in `bin/` and `lib/`.
