...
5. **Write the brief, then summon.**
   ...
   **Brief path check:** Before summoning, confirm every repo path cited in the brief is git-visible to a worker worktree by running `bin/advisor-check-brief-paths` - an untracked or gitignored path is invisible in the worktree and the worker will silently substitute guesswork.
