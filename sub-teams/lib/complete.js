...
const outputStr = args['output'];

if (!runDir || !taskId || outputStr === undefined) {
  console.log(JSON.stringify({ error: '--run-dir, --task-id, and --output are required' }));
  process.exit(1);
}
  ...
  while (true) {
    try {
      ...
      try {
        ...
        task.status = 'done';
        task.output = JSON.parse(outputStr);
      ...
      if (Date.now() >= deadline) throw new Error('complete: lock timeout');
