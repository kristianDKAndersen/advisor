...
// "Terminal signal" equality for the distinct-failure detector: no work done
// (both rounds touched zero files), or an identical test_state.output_tail.
// Equal-but-non-empty files_changed is NOT a no-work signal — iterating on
// one artifact touches the same files every round by definition.
function sameTerminalSignal(a, b) {
  ...
  const noFilesDelta = aFiles.length === 0 && bFiles.length === 0;
...
function decide(state, options = {}) {
  ...
  if (category === 'pane-death') {
    if (classify(round) === 'transient') {
      const rounds = state.rounds;
      let priorRetries = 0;
      for (let i = rounds.length - 2; i >= 0 && rounds[i].failure_category === 'pane-death'; i--) {
        priorRetries++;
      }
      if (priorRetries < PANE_DEATH_RETRY_LIMIT) {
        return { status: 'continue', action: 'retry', escalation: null };
      }
    }
    return {
