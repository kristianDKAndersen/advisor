...
// Pure helper: returns true when count strictly exceeds budget (budget=0 means unlimited).
function checkToolBudget(budget, count) {
  if (!budget || budget <= 0) return false;
  return count > budget;
}

module.exports = { canonicalHash, checkDuplicate, resetState, isProtectedWrite, bashWritesToProtected, checkToolBudget };
...
if (require.main === module) {
  ...
  process.stdin.on('end', () => {
    ...
    // Tool-budget enforcement: count every PreToolUse call under a __total__ key.
    // Block when the count exceeds ADVISOR_TOOL_BUDGET (0 / unset = unlimited).
    const budgetEnv = process.env.ADVISOR_TOOL_BUDGET;
    if (budgetEnv && sid) {
      const budget = parseInt(budgetEnv, 10);
      if (budget > 0) {
        const totalSig = '__total__';
        const totalCount = _incrementCount(sid, totalSig);
        if (checkToolBudget(budget, totalCount)) {
          process.stdout.write(
            `tool-guard: tool budget exceeded — limit is ${budget}, this is call #${totalCount}.\n` +
            `Send your result now via channel.js with verdict=blocked.\n`
          );
          process.exit(2);
        }
      }
