...
//            onAllComplete }
//
// onAllComplete(workers, report) — optional async callback invoked once after
// all workers have reached a terminal state (result/terminated/silent) and after
// session-report.json has been written to outputDir (if set), but BEFORE the
// function returns. Receives the final workers array and the assembled report
// object. The return value of the callback is ignored; throw to surface errors.
// Omitting the option leaves existing behavior unchanged.
//
...
async function runParallel(briefs, options = {}) {
  ...
  // Provision all worker sessions via bin/summon (headless, no timeline — automated fan-out).
  const workers = [];
  for (const brief of briefs) {
    const args = ['--agent', brief.agent, '--task', brief.task, '--goal', brief.goal, '--headless', '--no-timeline'];
    ...
    if (braceIdx === -1) throw new Error(`bin/summon for agent=${brief.agent} produced no JSON. stdout:\n${stdout}`);
    const meta = JSON.parse(stdout.slice(braceIdx));
    workers.push({
      sid: meta.sid,
      agent: brief.agent,
  ...
  process.stderr.write(`[parallel] all workers done — total elapsed: ${formatDuration(totalMs)}\n`);
  ...
  const report = {
    ...
    endedAt,
    elapsedMs: totalMs,
    elapsedFormatted: formatDuration(totalMs),
  };
  ...
  if (typeof onAllComplete === 'function') {
    await onAllComplete(workers, report);
  }
