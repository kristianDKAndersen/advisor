...
// Labels the two sides only A/B with a randomized which-is-which mapping.
// Never reveals the mapping to the critic; never passes the builder's result
// summary — only artifact paths and worktree_path, read from round_state.
function buildCriticBrief(state, roundRecord, worktreePath, randomFn) {
  ...
  const artifactsStr = (roundRecord.artifacts || []).join(', ') || '(no new files; inspect the worktree)';
  const lines = [
    `Goal: ${state.goal}`,
    `Artifact ${candidateLabel} (paths): ${artifactsStr}`,
    `Artifact ${barLabel}: ${describeBar(state.bar)}`,
    `Worktree path (both artifacts are on disk under this path): ${worktreePath}`,
    'Read both artifacts from disk yourself. Judge which of A or B better satisfies the goal.',
    'Return exactly one single biggest gap for whichever side is weaker.',
  ];
...
async function pollUntilTypeReal(outbox, targetType, timeoutMs = 30 * 60 * 1000, pollMs = 1000) {
...
async function defaultSpawnCritic(brief, execFileSyncFn = childProcess.execFileSync) {
  ...
  const resultMsg = await pollUntilTypeReal(meta.outbox, 'result');
  if (!resultMsg) throw new Error('loop-critic timed out without delivering a result');
...
async function runRound(state, options = {}) {
  ...
  if (!state.worktree_path) state.worktree_path = wt.path;
  ...
  const failureCategory = classifyFailure(builderWorker);
  ...
  if (roundRecord.builder_verdict !== 'blocked') {
    ...
    const criticResult = await spawnCriticFn({
      agent: 'loop-critic',
      task: briefText,
      goal: `Blind A/B judgment recorded to scores.json for goal: ${state.goal}`,
    });
    ...
    // Requirement 7: objective condition cannot be overridden by the critic's
    // blind judgment for acceptance-tests / metric bars.
    if (roundRecord.ab_verdict && roundRecord.ab_verdict.winner === 'candidate') {
      if ((state.bar.type === 'acceptance-tests' || state.bar.type === 'metric') &&
          !checkObjectiveCondition(state.bar, roundRecord)) {
        roundRecord.ab_verdict.winner = 'bar';
      }
    }
    // The critic is blind to the A/B label mapping and cannot know which
    // side is the candidate, so its self-reported overall_pass is only an
    // impression. overall_pass is the driver's to compute from the
    // authoritative, post-downgrade winner.
