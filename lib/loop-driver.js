...
// Coarse in-memory first pass: maps runParallel's four terminal statuses onto
// the census categories (hit-timeout/api-stall/pane-death/launch-death/blocked)
// from data already in hand. This is one tier of a two-tier design: the call
// site (~lines 362-377) defers to the on-disk classifySessionDir when there is
// no result envelope, and surfaces any disagreement rather than silently
// discarding it when there is one.
...
function checkObjectiveCondition(bar, roundRecord) {
  ...
  if (bar.type === 'metric') {
    // defensive only: bar-type "metric" is rejected at declaration time in
    // lib/loop-bar.js (no field or pipeline ever writes metric_value), so this
    // branch should be unreachable in practice. Left fail-closed rather than
    // removed, since a value that cannot be produced must never be treated as met.
...
// The critic's job is to independently verify a visible test suite's result
// rather than trust it — re-running the suite, reading artifacts, and
// checking for a hang all cost real tool calls. The generic tier-based
// default (25, see lib/summon.js TIER_BUDGETS) ships with "Plan carefully...
// No incremental solutions" frugality language that a live eval run showed
// pushes the critic toward rubber-stamping instead of verifying. Give it
// real headroom so independent verification is never the "expensive" path.
const CRITIC_TOOL_BUDGET = 60;

async function defaultSpawnCritic(brief, options = {}) {
  const {
    execFileSyncFn = childProcess.execFileSync,
    pollUntilTerminalFn = pollUntilTerminalReal,
    toolBudget = CRITIC_TOOL_BUDGET,
  } = options;
  const meta = summonAgentReal(
    ['--agent', brief.agent, '--task', brief.task, '--goal', brief.goal, '--tool-budget', String(toolBudget)],
    execFileSyncFn
  );
  const msg = await pollUntilTerminalFn(meta.outbox);
