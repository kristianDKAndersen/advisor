...
let _db = null;
let _dbPath = null;
let _rebuilding = false;         // R1: guard so a drift-triggered rebuild's internal db() calls don't recurse / self-deadlock on the migration lock
let _migrationAttempted = false; // R1: at most one schema-migration attempt per process per db path

// R1: detect a pre-`routing` vault — a `notes` table lacking the `routing`
// column, or a `notes_fts` whose stored DDL predates the `routing` FTS column.
// The FTS check reads sqlite_master text because `CREATE ... IF NOT EXISTS`
// never upgrades an already-existing table, so an old two-column FTS survives.
function _hasSchemaDrift(d) {
  try {
    const notesCols = d.prepare(`PRAGMA table_info(notes)`).all().map(r => r.name);
    if (!notesCols.includes('routing')) return true;
    const row = d.prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name='notes_fts'`).get();
    if (!row || !row.sql || !/\brouting\b/.test(row.sql)) return true;
    return false;
  } catch (_) { return false; } // undeterminable → fail-safe: do NOT trigger a destructive rebuild
}

function db() {
  const currentPath = dbPath();
  if (_db && _dbPath === currentPath) return _db;
  if (_db && _dbPath !== currentPath) { try { _db.close(); } catch (_) {} _db = null; _migrationAttempted = false; }
  _dbPath = currentPath;
  ...
  _db.exec(`
    CREATE TABLE IF NOT EXISTS notes (
      ...
      established TEXT, gap TEXT, plan_ref TEXT, body TEXT, routing TEXT
    );
    CREATE VIRTUAL TABLE IF NOT EXISTS notes_fts
      USING fts5(path UNINDEXED, body, routing, content='notes', content_rowid='rowid');
  ...
  _db.exec(`
    CREATE TRIGGER IF NOT EXISTS notes_ai AFTER INSERT ON notes BEGIN
      INSERT INTO notes_fts(rowid, path, body, routing) VALUES (new.rowid, new.path, new.body, new.routing);
    END;
    CREATE TRIGGER IF NOT EXISTS notes_ad AFTER DELETE ON notes BEGIN
      INSERT INTO notes_fts(notes_fts, rowid, path, body, routing) VALUES('delete', old.rowid, old.path, old.body, old.routing);
    END;
  ...
  if (!cols.includes('confidence')) {
    try { _db.exec(`ALTER TABLE notes ADD COLUMN confidence REAL DEFAULT 1.0`); } catch (_) {}
  }
  if (!cols.includes('superseded_by')) {
    try { _db.exec(`ALTER TABLE notes ADD COLUMN superseded_by TEXT DEFAULT NULL`); } catch (_) {}
  }
  // migrate: episode_goals table + FTS5 for fuzzy episode retrieval
  const epTables = _db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='episode_goals'`).all();
  if (!epTables.length) {
    _db.exec(`
      CREATE TABLE IF NOT EXISTS episode_goals (
        rowid INTEGER PRIMARY KEY,
        sid TEXT UNIQUE NOT NULL,
        goal TEXT
      );
      CREATE VIRTUAL TABLE IF NOT EXISTS episode_goals_fts
        USING fts5(sid UNINDEXED, goal, content='episode_goals', content_rowid='rowid');
      CREATE TRIGGER IF NOT EXISTS ep_goals_ai AFTER INSERT ON episode_goals BEGIN
        INSERT INTO episode_goals_fts(rowid, sid, goal) VALUES (new.rowid, new.sid, new.goal);
      END;
      CREATE TRIGGER IF NOT EXISTS ep_goals_ad AFTER DELETE ON episode_goals BEGIN
        INSERT INTO episode_goals_fts(episode_goals_fts, rowid, sid, goal) VALUES ('delete', old.rowid, old.sid, old.goal);
      END;
      CREATE TRIGGER IF NOT EXISTS ep_goals_au AFTER UPDATE ON episode_goals BEGIN
        INSERT INTO episode_goals_fts(episode_goals_fts, rowid, sid, goal) VALUES ('delete', old.rowid, old.sid, old.goal);
        INSERT INTO episode_goals_fts(rowid, sid, goal) VALUES (new.rowid, new.sid, new.goal);
      END;
    `);
  ...
  // R1: transparent migration for pre-`routing` vaults. The CREATE ... IF NOT
  // EXISTS statements above cannot upgrade an existing notes/notes_fts, and the
  // ALTER migrations never add `routing` nor rebuild the two-column FTS — so on
  // an old vault the routing-aware triggers push a 4-value row into a 3-column
  // FTS and _upsertIndex binds a nonexistent $routing column, both swallowed,
  // surfacing as SILENT total-recall loss. The .md files are the source of
  // truth; rebuildIndex() drops the DB and re-walks them under the current
  // schema (preserving embeddings), so it is the repair primitive.
  //   RE-ENTRANCY: rebuildIndex() calls db() internally, so `_rebuilding` makes
  //     those re-entrant opens skip detection — which also prevents a self-
  //     deadlock on the single-holder migration lock below.
  //   CONCURRENCY: many workers can each detect drift at once; they serialise on
  //     one FS mkdir-lock and re-probe the on-disk schema under it, so only the
  //     first rebuilds and the rest observe the finished upgrade. withNoteLock
  //     throws after 5s; a 1,835-note rebuild exceeds that, so a waiting peer
  //     fails open (logs, returns a best-effort handle) and — because the lock
  //     was never held, _migrationAttempted stays false — retries on its next
  //     open, by which point the holder has finished. IDEMPOTENT: a current-
  //     schema vault reports no drift, so re-running is a no-op.
  if (!_rebuilding && !_migrationAttempted && _hasSchemaDrift(_db)) {
    _rebuilding = true;
    try {
      withNoteLock('__schema_migration__', () => {
        // Our handle may reference a now-unlinked inode if a peer rebuilt while
        // we waited for the lock; probe the on-disk file for the live schema.
        try { if (_db) _db.close(); } catch (_) {}
        _db = null;
        let drifted = true;
        try { const probe = new Database(currentPath); drifted = _hasSchemaDrift(probe); probe.close(); } catch (_) { drifted = true; }
        _migrationAttempted = true; // holding the lock == this process's single attempt
        if (drifted) rebuildIndex();
      });
    } catch (e) {
      console.error(`[vault] schema migration failed: ${e && e.message}`);
    } finally {
      _rebuilding = false;
    }
    // Guarantee a live handle without recursing back through drift detection.
    if (!_db) {
      try { _dbPath = currentPath; _db = new Database(currentPath); _db.exec('PRAGMA journal_mode=WAL'); } catch (_) {}
    }
  }
  return _db;
}
  ...
  try {
    ...
    d.prepare(`
      INSERT INTO notes (path, type, sid, seq, agent, repo, created_at,
                         material, next_action, established, gap, plan_ref, body, routing, due_date,
                         fetched_at, published_at, content_hash, t_valid, t_invalid, task_hash,
                         status, confidence, superseded_by)
    ...
    `).run({
      ...
      $routing: [fm.task_type, Array.isArray(fm.tags) ? fm.tags.join(' ') : (fm.tags || '')].filter(Boolean).join(' '),
      ...
      $confidence: fm.confidence !== undefined && fm.confidence !== '' ? parseFloat(fm.confidence) : 1.0,
    ...
    // R2: the note write already succeeded and the index is advisory, so we
    // still swallow — but surface the cause rather than hiding schema/SQL faults
    // (a bare catch here was co-responsible for the silent recall failure).
    console.error(`[vault] _upsertIndex failed for ${relPath}: ${e && e.message}`);
  }
}
...
// Default review horizon for auto-generated lessons. Manual lessons cluster
// at 14/30/92 days; 90d matches the 92d cluster without hand-tuning per note.
const LESSON_DUE_HORIZON_DAYS = 90;
...
function writeLesson(record) {
  ...
  const createdAt = record.ts_iso || new Date(ts * 1000).toISOString();
  const dueDate = record.due_date
    || new Date(new Date(createdAt).getTime() + LESSON_DUE_HORIZON_DAYS * 86400000).toISOString().slice(0, 10);
  const body = [
...
// D2: build an FTS5 MATCH expression. A bare multi-token query is joined with OR
// (previously space-joined => implicit AND, so MORE keywords returned FEWER hits
// and CLAUDE.md Step 5's "pass 3 keywords" was self-defeating). Queries that
// already use FTS operators/phrases/groups are passed through verbatim via the
// legacy escaper, so callers keep full boolean control.
// unicode61 tokenizer only, no porter stemming ('verified' != 'verification').
// Measured 2026-08-25 against the real 1921-note vault corpus (eval/calibration.md):
// decided AGAINST enabling porter stemming. Recall gain was real but modest, while
// ~10% of top-10 result slots churned per query from IDF-driven reranking alone —
// including queries with no morphology problem at all — and a tokenizer swap on
// notes_fts is a destructive migration (DROP + full reindex, no in-place upgrade).
// If cross-form recall later becomes a documented recurring pain point, the
// preferred path is an ADDITIVE porter-stemmed shadow index queried only when the
// primary search returns very few hits — not a tokenizer swap on notes_fts.
function _buildMatchQuery(text) {
  const raw = (text || '').trim();
  if (!raw) return raw;
  if (/["()]/.test(raw) || /\b(?:OR|AND|NOT|NEAR)\b/.test(raw)) return _escapeQuery(raw);
  const tokens = raw.split(/\s+/).filter(Boolean);
  if (tokens.length <= 1) return _escapeQuery(raw);
  return tokens.map(t => (t.includes('-') ? `"${t}"` : t)).join(' OR ');
}

// R3: last-resort fallback for arbitrary free text that FTS5's operator grammar
// cannot parse (unbalanced quotes/parens, bare '#'/'.'/'%'/'+' punctuation,
// dangling AND/OR/NOT, unknown column refs, etc). Requotes every whitespace
// token as a literal FTS5 phrase (doubling embedded quotes) so the query never
// engages FTS5 operator syntax at all — the same unicode61 tokenizer that
// indexed the note also tokenizes the phrase, so "C++"/"node.js"/"c#" still
// match the indexed terms. Tokens with no alphanumeric content (e.g. a bare
// '+') are dropped; if nothing survives, returns '' so the caller can bail
// to [] without ever hitting SQLite again.
function _fallbackMatchQuery(text) {
...
// D3: pool-independent absolute relevance in [0,1), derived from the raw FTS5
// bm25 score (SQLite returns <= 0; more negative = stronger match). Unlike the
// within-pool min-max `bm25norm` used for RANKING, this does NOT depend on the
// other rows in the pool, so the least-bad of a terrible pool still scores low.
// This is the value the opt-in floor gates on — never the blended score.
const BM25_RELEVANCE_SCALE = 6.0;
function _bm25Relevance(bm25raw) {
  const mag = Math.max(0, -bm25raw);
  return 1 - Math.exp(-mag / BM25_RELEVANCE_SCALE);
}

const RECENCY_HALFLIFE_MS = 30 * 86400000;
const CONFIDENCE_HALFLIFE_MS = 90 * 86400000;
const CONFIDENCE_FLOOR = 0.1;

function searchNotes(text, limit = 10, opts = {}) {
  try {
    if (opts.legacy) {
      ...
      `);
      try {
        return legacyStmt.all(_escapeQuery(text), limit);
      } catch (e) {
        const fallback = _fallbackMatchQuery(text);
        if (!fallback) return [];
        return legacyStmt.all(fallback, limit);
      }
    }
    const poolSize = Math.max(30, limit * 3);
    const placeholders = DISMISSED_STATUSES.map(() => '?').join(',');
    const mainStmt = db().prepare(`
      SELECT n.path, n.sid, n.type, n.created_at, n.material, n.status, n.confidence,
             snippet(notes_fts, 1, '<b>', '</b>', '…', 20) AS snippet,
             bm25(notes_fts) AS bm25raw
      FROM notes_fts f JOIN notes n ON n.path = f.path
    ...
    let rows;
    try {
      rows = mainStmt.all(_buildMatchQuery(text), ...DISMISSED_STATUSES, poolSize);
    } catch (e) {
      const fallback = _fallbackMatchQuery(text);
      if (!fallback) return [];
      rows = mainStmt.all(fallback, ...DISMISSED_STATUSES, poolSize);
    }

    if (!rows.length) return [];

    const bmVals = rows.map(r => r.bm25raw);
    const minBm = Math.min(...bmVals), maxBm = Math.max(...bmVals);
    const bmRange = maxBm - minBm;
    const now = Date.now();

    const scored = rows.map(r => {
      const bm25norm = bmRange > 0 ? (maxBm - r.bm25raw) / bmRange : 1;
      ...
      const recency = ageMs === null ? 0 : Math.pow(0.5, ageMs / RECENCY_HALFLIFE_MS);
      const storedConfidence = (typeof r.confidence === 'number' && !Number.isNaN(r.confidence)) ? r.confidence : 1.0;
      const confidence = ageMs === null
        ? CONFIDENCE_FLOOR
        : Math.max(CONFIDENCE_FLOOR, storedConfidence * Math.pow(0.5, ageMs / CONFIDENCE_HALFLIFE_MS));
      let score = 0.5 * bm25norm + 0.3 * recency + 0.2 * confidence;
      if (r.status === 'stale') score *= 0.5;
      return { path: r.path, sid: r.sid, type: r.type, created_at: r.created_at, material: r.material, snippet: r.snippet, score, _relevance: _bm25Relevance(r.bm25raw) };
    });

    scored.sort((a, b) => b.score - a.score);

    // D3: opt-in relevance floor. DEFAULT OFF — active only when a caller passes
    // a positive numeric opts.minRelevance (lib/summon.js auto-recall does; the
    // interactive `bin/advisor-vault search` path does not, so it never silently
    // suppresses). Gates on the pool-independent RELEVANCE component, never the
    // blended score, so freshness/confidence cannot rescue an irrelevant note.
    const minRelevance = (typeof opts.minRelevance === 'number' && Number.isFinite(opts.minRelevance))
      ? opts.minRelevance : 0;
    let out = scored;
    if (minRelevance > 0) {
      out = scored.filter(s => s._relevance >= minRelevance);
      if (out.length === 0 && scored.length > 0) {
        const best = Math.max(...scored.map(s => s._relevance));
        // Observability: never suppress silently (same defect class as the old
        // undocumented task.length>=20 gate).
        console.error(`[vault] recall floor suppressed all candidates: candidates_considered=${scored.length} best_relevance=${best.toFixed(4)} threshold=${minRelevance}`);
      }
    }
    return out.slice(0, limit).map(({ _relevance, ...rest }) => rest);
  } catch (e) {
    // R2: surface genuine SQL/schema failures instead of the bare swallow that
    // hid the pre-`routing` schema drift as silent total-recall loss. The
    // fail-open contract is preserved — callers still receive [].
    console.error(`[vault] searchNotes failed: ${e && e.message}`);
    return [];
  }
}

function indexEpisodeGoal(sid, goalText) {
  try {
    db().prepare(`INSERT INTO episode_goals (sid, goal) VALUES (?,?)
      ON CONFLICT(sid) DO UPDATE SET goal=excluded.goal`).run(sid, goalText || '');
  } catch (_) {}
}

function searchEpisodeGoals(goalText, limit = 10) {
  try {
    const tokens = goalText.split(/\s+/)
      .slice(0, 50)
    ...
    return db().prepare(`
      SELECT e.sid
      FROM episode_goals_fts f JOIN episode_goals e ON e.rowid = f.rowid
      WHERE episode_goals_fts MATCH ? ORDER BY rank LIMIT ?
    `).all(query, limit).map(r => r.sid);
  } catch (_) { return []; }
}
...
// ── Staleness scan: flag notes whose referenced absolute file paths are
// deleted or modified after the note was written ────────────────────────────
const _ABS_PATH_RE = /(?:\/Users\/|\/home\/)[^\s'"`)\]]+/g;

function scanStale({ dryRun = false } = {}) {
  const d = db();
  const rows = d.prepare(`SELECT path, body, created_at, status FROM notes WHERE type IN ('synthesis','lesson','project')`).all();
  let scanned = 0, markedStale = 0, cleared = 0;
  const report = [];
  for (const row of rows) {
    scanned++;
    const refs = [...new Set((row.body || '').match(_ABS_PATH_RE) || [])]
      .map(p => p.replace(/[.,;:)\]]+$/, ''));
    if (!refs.length) continue;
    const createdMs = row.created_at ? Date.parse(row.created_at) : NaN;
    if (!Number.isFinite(createdMs)) continue;

    let isStaleNow = false;
    for (const ref of refs) {
      if (!fs.existsSync(ref)) { isStaleNow = true; break; }
      try {
        if (fs.statSync(ref).mtimeMs > createdMs) { isStaleNow = true; break; }
      } catch (_) { isStaleNow = true; break; }
    }

    if (isStaleNow && row.status !== 'stale') {
      if (!dryRun) setStatus(row.path, 'stale');
      markedStale++;
      report.push({ path: row.path, action: 'marked-stale' });
    } else if (!isStaleNow && row.status === 'stale') {
      if (!dryRun) setStatus(row.path, '');
      cleared++;
      report.push({ path: row.path, action: 'cleared' });
    } else {
      report.push({ path: row.path, action: 'unchanged' });
    }
  }
  return { scanned, markedStale, cleared, report };
}

// ── Supersession: mark an old note as replaced by a newer one ───────────────
function supersedeNote(oldName, newName) {
  withNoteLock(oldName, () => {
    const note = readNote(oldName);
    if (!note) return;
    _assertSingleLineFrontmatter(oldName);
    writeNote(oldName, { ...note.fm, superseded_by: newName }, note.body);
    try { db().prepare(`UPDATE notes SET superseded_by = ? WHERE path = ?`).run(newName, oldName); } catch (_) {}
  });
}
