  ...
  try {
    ...
    d.transaction(() => {
      ...
      for (const t of targets) {
        if (_isValidLinkTarget(t, bnames)) ins.run(relPath, t);
      }
    })();
...
function _isValidLinkTarget(t, knownBasenames) {
  if (!t) return false;
  if (t.startsWith('http://') || t.startsWith('https://')) return true;
  if (_SID_EXACT.test(t) || _SID_SEQ_EXACT.test(t)) return true;
  const bn = t.replace(/^.*\//, '').replace(/\.md$/, '');
  return knownBasenames.has(bn) || knownBasenames.has(t);
}
...
export function backfillVerdicts(dryRun = false, runsDir = null) {
...
export async function embedNotes({ limit = 0, threshold = 0.75, force = false, onlyChanged = false, _pipelineFactory = null } = {}) {
  const d = db();
  ...
  for (let i = 0; i < vecList.length; i++) {
    for (let j = i + 1; j < vecList.length; j++) {
      const { path: pa, vec: va } = vecList[i];
      const { path: pb, vec: vb } = vecList[j];
      if (_cosine(va, vb) < threshold) continue;

      // Canonical order: path_a < path_b
      const [path_a, path_b] = pa < pb ? [pa, pb] : [pb, pa];
      const target_b = path_b.replace(/^.*\//, '').replace(/\.md$/, '');
      const target_a = path_a.replace(/^.*\//, '').replace(/\.md$/, '');

      // Skip if any link already exists between these two notes (any kind)
