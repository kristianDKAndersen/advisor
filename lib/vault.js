...
function searchNotes(text, limit = 10, opts = {}) {
  try {
    ...
    const rows = db().prepare(`
      ...
      ORDER BY rank LIMIT ?
    `).all(_escapeQuery(text), ...DISMISSED_STATUSES, poolSize);
    ...
    const scored = rows.map(r => {
      ...
      return { path: r.path, sid: r.sid, type: r.type, created_at: r.created_at, material: r.material, snippet: r.snippet, score };
    });
