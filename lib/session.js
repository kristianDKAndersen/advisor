...
// Filter helper: removes sessions flagged as test runs from a session list.
// A session is a test run when meta.json has isTestSession: true.
function filterTestSessions(sessions) {
  return (sessions || []).filter(s => !s.isTestSession);
}
