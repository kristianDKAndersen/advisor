...
//   channel.js recv --file <path> [--after <seq>] [--json] [--compact[=lossy]]
...
const { updateSessionState, readMeta, ADVISOR_ROOT } = require('./session');
const { persistTerminal } = require('./terminal-persist');

// Stale-lock threshold: mirror SESSION_LOCK_STALE_MS in lib/session.js so the
// .seq.lock recovery uses the exact same window as withSessionLock.
...
// Parses complete newline-terminated JSON lines out of `text`, skipping a
// trailing unterminated line (an in-progress write, not corruption).
function parseCompleteJsonLines(text, warnLabel) {
  ...
  for (const line of text.slice(0, lastNewline).split('\n')) {
    ...
    try {
      ...
      process.stderr.write(`[channel] ${warnLabel}skipping malformed line: ${line.slice(0, 120)}\n`);
    }
  }
  return out;
}
...
function readAll(p) {
  ...
  return parseCompleteJsonLines(content, '');
}
...
class Tail {
  ...
  read(filePath) {
    ...
    try {
      ...
      const lastNewline = text.lastIndexOf('\n');
      if (lastNewline < 0) return [];
      this._offsets.set(filePath, offset + lastNewline + 1);
      return parseCompleteJsonLines(text, 'Tail: ');
    } finally {
      fs.closeSync(fd);
    }
  }
...
async function cli() {
  ...
  if (cmd === 'synthesize') {
    ...
    // --worker-sid disambiguates which pending decomposition entry this
    // synthesize call belongs to. Without it, fall back to legacy
    // first-pending-wins matching for backward compatibility.
    const workerSid = (args['worker-sid'] && args['worker-sid'] !== true) ? args['worker-sid'] : null;
    updateSessionState(args.sid, s => {
      const decomposition = s.decomposition || [];
      let entry = workerSid
        ? decomposition.find(d => d.synthesis_seq == null && d.sid === workerSid)
        : decomposition.find(d => d.synthesis_seq == null);
      const isNew = !entry;
      if (isNew) entry = workerSid ? { sid: workerSid } : {};
    ...
    process.stdout.write(`synthesis recorded: sid=${sid} seq=${seq} material=${args.material}\n`);
    {
      ...
      // Reachable lesson-extraction nudge. Repairs the guard deleted in cf30392,
      // which gated on the never-passed --verdict CLI flag (unreachable). The
      // worker verdict lives on the persisted result envelope, not on args.
      // Fires only on the 2nd+ blocked verdict this session (one is noise).
      if (persistResultMsg?.body?.verdict === 'blocked') {
        const blockedCount = persistAllMsgs.filter(
          m => m.type === 'result' && m.seq <= Number(args.seq) && m.body?.verdict === 'blocked'
        ).length;
        if (blockedCount >= 2) {
          const agent = (persistResultMsg.from && persistResultMsg.from !== true)
            ? persistResultMsg.from : '<agent>';
          process.stdout.write(
            '\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n' +
            'LESSON EXTRACTION REQUIRED — 2nd blocked verdict for this session\n' +
            `(sid=${sid}, seq=${seq}, verdict=blocked, blocked_count=${blockedCount})\n` +
            '\nRun the extract-lesson skill before closing this session:\n\n' +
            `  /extract-lesson \\\n` +
            `    --synthesis-log ${path.join(runsRoot(), sid, 'synthesis.log')} \\\n` +
            `    --synthesis-seq ${seq} \\\n` +
            `    --agent ${agent}\n` +
            '\nWrites to ~/.advisor/vault/lessons/' + `${sid}-${agent}-${seq}.md\n` +
            '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n'
          );
        }
      }
    }
    // Enqueue doc-update entry for the background doc-agent.
    // Ordering: MUST run before close-worker-tab and workspace cleanup — both steps
    // that follow may destroy the worktree, so the git diff must happen first.
    // Uses runsRoot() (respects ADVISOR_RUNS_ROOT) rather than os.homedir() so tests
    // can redirect via the env var — functionally identical in production.
    try {
      const worktreePath = path.join(runsRoot(), sid, 'workspace');
      let modifiedFiles = [];
      try {
        const diffOut = execFileSync(
          'git', ['-C', worktreePath, 'diff', '--name-only', 'HEAD'],
    ...
    // Worker sessions execute their whole task as one agent turn and
    // self-terminate before the turn ends, so the Stop hook
    // (.claude/hooks/stop-telemetry.js) never fires inside them. Accrue the
    // usage row here instead, before the tab (and the transcript-generating
    // Claude Code process behind it) goes away. Non-fatal: a missing
    // session-map entry or transcript must never abort synthesize.
    try {
      const { accrueForSid } = require('./telemetry-backfill');
      const workerMeta = readMeta(sid);
      const telemetryResult = accrueForSid(sid, workerMeta && workerMeta.workspace);
      if (!telemetryResult.ok) {
        process.stderr.write(`telemetry backfill skipped for sid=${sid}: ${telemetryResult.reason}\n`);
      }
    } catch (e) {
      process.stderr.write(`telemetry backfill error for sid=${sid}: ${e.message}\n`);
    }
    ...
    // Remove the per-worker git worktree (coder agents) or plain workspace dir
    // (non-coder agents) so .git/worktrees entries and workspace dirs do not
    // accumulate across sessions. Fail-open: a cleanup failure must never abort
    // synthesize or change its exit code.
    try {
      const wPath = path.join(runsRoot(), sid, 'workspace');
      const root = runsRoot();
      if (wPath && wPath.length > 0 && fs.existsSync(wPath) &&
          wPath.startsWith(root + path.sep) && wPath.includes(sid)) {
        const cleanupMeta = readMeta(sid);
        // Single source of truth: which agents run in a git worktree (and thus
        // need `git worktree remove`, not a plain rmSync). Lazy require avoids a
        // load-time cycle (summon.js requires this module at top level).
        const { WORKTREE_AGENTS } = require('./summon');
        if (cleanupMeta && WORKTREE_AGENTS.has(cleanupMeta.agent)) {
          const repo = cleanupMeta.repo || ADVISOR_ROOT;
          try { execFileSync('git', ['-C', repo, 'worktree', 'remove', '--force', wPath], { stdio: 'ignore' }); } catch (_) {}
          try { execFileSync('git', ['-C', repo, 'worktree', 'prune'], { stdio: 'ignore' }); } catch (_) {}
          try { execFileSync('git', ['-C', repo, 'branch', '-D', `ws/${sid}`], { stdio: 'ignore' }); } catch (_) {}
        } else {
          fs.rmSync(wPath, { recursive: true, force: true });
        }
      }
    } catch (_) {}
    return;
  ...
  if (cmd === 'recv') {
    ...
    // --compact: opt-in presentation transform for re-reading long histories
    // into model context (see lib/compactor.js compactChannelRecords). Pure
    // read-side: the on-disk JSONL and default recv output are unchanged.
    if (args.compact) {
      const { compactChannelRecords, compactChannelRecordsLossy } = require('./compactor');
      if (args.compact === 'lossy') {
        console.log(compactChannelRecordsLossy(msgs));
      } else if (args.compact === true) {
        console.log(compactChannelRecords(msgs));
      } else {
        console.error(`unknown --compact mode: ${args.compact} (use --compact or --compact=lossy)`);
        process.exit(1);
      }
    } else if (args.json) console.log(JSON.stringify(msgs));
