...
function append(p, msg) {
  ensureFile(p);
  return acquireSeqLock(path.dirname(p), p, { ts: Date.now() / 1000, ...msg });
}
...
async function cli() {
  ...
  if (cmd === 'synthesize') {
    ...
    {
      const home = process.env.HOME || process.env.USERPROFILE;
      const checkpointsDir = path.join(home, '.advisor', 'runs', sid, 'checkpoints');
    ...
    {
      const persistHome = process.env.HOME || process.env.USERPROFILE;
      const persistChannelDir = path.join(persistHome, '.advisor', 'runs', sid, 'channel');
...
module.exports = { append, readAll, readAfter, tail, acquireSeqLock, Tail, appendSyntheticIfAbsent };
