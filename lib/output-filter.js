...
// Assertion-detail lines: Expected/Received/Actual values and stack frames
const RE_ASSERT_DETAIL = /^\s*(Expected|Received|Actual|at )\b/;
// Diff markers: unified diff +/- lines, carets, assertion bullets
const RE_DIFF_MARKER = /^\s*[+\-]\s|^[-+]{3}\s|^\s*\^+\s*$|^\s*[●✗✕×]\s/;

function scoreLine(clean) {
  if (!clean) return 0.1;
  if (RE_HIGH.test(clean)) return 1.0;
  if (RE_ASSERT_DETAIL.test(clean)) return 0.8;
  if (RE_DIFF_MARKER.test(clean)) return 0.7;
...
function filter(rawText, { lineBudget = 80, minBytes = 2048 } = {}) {
  ...
  // Adjacency: any item within 2 positions of a score>=1.0 item is promoted to >=0.5
  const n = items.length;
  for (let i = 0; i < n; i++) {
    if (items[i].score >= 1.0) {
      for (let j = Math.max(0, i - 2); j <= Math.min(n - 1, i + 2); j++) {
        if (j !== i && items[j].score < 0.5) items[j].score = 0.5;
      }
    }
  }
