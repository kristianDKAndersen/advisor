...
// .claude/hooks/stop-telemetry.js.
async function main() {
  let raw = '';
...
// compactChannelRecordsLossy(records) -> string
//   Opt-in LOSSY variant (recv --compact=lossy). Collapses each contiguous
//   run of 3+ 'progress' records: the run's first and last records are kept,
//   the middle is replaced by a synthetic marker record (type 'elided', body
//   'N progress records elided (seq A..B)') so the elision is always explicit,
//   never a silent drop. All other record types (task, result, guidance,
//   question, ...) and the kept progress records pass through VERBATIM into
//   the lossless columnar encoding above, so expandChannelCompact recovers
//   them exactly. A '#lossy elided=N of=M' header line identifies the output
//   as a summary; expandChannelCompact ignores unknown '#' lines.

function compactChannelRecordsLossy(records) {
  if (!Array.isArray(records)) records = [];
  const kept = [];
  let elided = 0;
  let i = 0;
  while (i < records.length) {
    if (String(records[i].type) !== 'progress') {
      kept.push(records[i]);
      i++;
      continue;
    }
    let j = i;
    while (j < records.length && String(records[j].type) === 'progress') j++;
    ...
    if (run.length <= 2) {
      ...
      elided += mid.length;
      const froms = new Set(mid.map(r => (r.from == null ? null : String(r.from))));
      kept.push(run[0]);
      kept.push({
        ts: mid[0].ts,
        type: 'elided',
        from: froms.size === 1 ? [...froms][0] : null,
        seq: mid[0].seq,
        body: `${mid.length} progress records elided (seq ${mid[0].seq}..${mid[mid.length - 1].seq})`,
      });
      kept.push(run[run.length - 1]);
    }
    i = j;
  }
  const lines = compactChannelRecords(kept).split('\n');
  lines.splice(1, 0, `#lossy elided=${elided} of=${records.length}`);
  return lines.join('\n');
}
