...
# --print so the bootstrap is consumed as the first user turn and tool calls
# execute non-interactively. Streamed stdout stays visible in the spawned
# Terminal tab. --permission-mode auto because no human is at the TUI to
# approve tool calls.
#
# The leading double-dash before the prompt is load-bearing: claude's
# --allowedTools is variadic (<tools...> per claude --help). Without the
# double-dash terminator, commander.js consumes the prompt argument as
# another tool name, claude sees no positional prompt, and either errors
# with --print or silently opens an empty TUI in interactive mode. That
# was the root cause of "worker spawned but did nothing".
# Verified 2026-05-07 / v2.1.132.
${modelExport}exec claude --print --permission-mode auto ${modelFlag}${allowedToolsFlag}-- "$(cat ${shesc(promptFile)})"
`
  );
