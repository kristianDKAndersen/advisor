...
function provisionOne(args, overrideSid) {
  ...
  const claudeCmd = tui
    ? `exec claude --permission-mode auto ${modelFlag}${allowedToolsFlag}-- "$(cat ${shesc(promptFile)})"`
    : `exec claude --print --permission-mode auto ${modelFlag}${allowedToolsFlag}-- "$(cat ${shesc(promptFile)})"`;
  fs.writeFileSync(
    launchScript,
    `#!/usr/bin/env bash
set -e
