...
// Pure helper: returns the MCP-approval settings fragment. An empty
// enabledMcpjsonServers whitelist makes Claude Code reject every .mcp.json
// server without prompting, keeping headless worker sessions non-interactive.
function buildMcpSettings() {
...
function provisionOne(args, overrideSid) {
  ...
  const WORKER_HOOKS_ALLOWLIST = new Set(['planner', 'researcher']);
  {
    ...
    settings.hooks = {
      ...
      PostToolUse: [
        {
          matcher: '',
          hooks: [{ type: 'command', command: 'node $ADV/lib/hooks/worker-trace.js' }],
        },
        {
          matcher: '',
          hooks: [{ type: 'command', command: 'bash $ADV/lib/hooks/worker-inbox-poll.sh' }],
        },
        {
          matcher: 'Bash',
          hooks: [{ type: 'command', command: 'bash $ADV/lib/hooks/worker-auto-close.sh' }],
        },
      ],
    };
    ...
    Object.assign(settings, buildMcpSettings());
  ...
  const claudeCore = `claude --permission-mode auto ${modelFlag}${allowedToolsFlag}-- "$(cat ${shesc(promptFile)})"`;
...
module.exports = { composeBootstrapPrompt, trustWorkspaceInClaudeConfig, resolveIntelligence, provisionOne, buildProtectedTestsEnv, buildMcpSettings };
