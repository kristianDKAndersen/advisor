...
// Overlay the agent's files (spawns/<agent>/) onto a git worktree, guarding the
// case-fold collision that silently destroys a repo's own root prompt.
//
...
// `claude.md` are the SAME inode, so a naive copy overwrites the tracked
// content. `git ls-files` prints paths case-SENSITIVELY, so the detection MUST
// fold case — a literal `CLAUDE.md` lookup would miss tracked `claude.md` (the
// case-fold IS the bug).
//
...
// On a genuine case-difference collision for the root prompt the original is
// copied to <dest>/.advisor-root-prompt.md BEFORE the overlay, a [summon]
// warning is emitted, and { collision:true, preservedPath } is returned. With
// no collision the body is byte-for-byte the original localCopyDir(agentSrc,
// dest) and returns { collision:false, preservedPath:null } — provably inert.
function overlayAgentFiles(agentSrc, dest, repo) {
  ...
  // The documented bug is the ROOT prompt: agent `CLAUDE.md` folding onto a
  // tracked `claude.md`. Fire ONLY on a genuine case difference — if the repo
  // tracks `CLAUDE.md` in the exact same casing, the overlay overwriting it is
  // pre-existing behaviour this fix must not change.
  const trackedRoot = trackedByLower.get('claude.md');
  if (
    trackedRoot &&
    trackedRoot !== 'CLAUDE.md' &&
    fs.existsSync(path.join(agentSrc, 'CLAUDE.md'))
  ) {
    const candidate = path.join(dest, '.advisor-root-prompt.md');
    try {
      fs.copyFileSync(path.join(dest, trackedRoot), candidate);
      collision = true;
      preservedPath = candidate;
      process.stderr.write(
        `[summon] overlay collision: repo tracks '${trackedRoot}' under different ` +
          `casing than the agent's 'CLAUDE.md'; on a case-insensitive filesystem they ` +
          `are one file, so the overlay would destroy the repo's own root prompt. ` +
          `Preserved the original to '${candidate}' before overlaying.\n`
      );
    } catch (e) {
      // Preservation is load-bearing; if it fails, do NOT claim the collision was
      // handled — surface the failure and fall through to the plain overlay.
      process.stderr.write(
        `[summon] warn: could not preserve tracked root prompt '${trackedRoot}': ${e.message}\n`
      );
    }
  }
  ...
  return { collision, preservedPath };
}
...
${rootPromptCollision ? `
NOTE: this repo tracks its own root orchestrator prompt under a case-differing
filename, which the overlay of your \`CLAUDE.md\` had to displace on this
case-insensitive filesystem. The displaced original is preserved verbatim at
\`<worktree>/.advisor-root-prompt.md\` — read that when you need the repo's own
root prompt (the \`CLAUDE.md\` in your working directory is YOUR role prompt).
` : ''}
...
function provisionOne(args, overrideSid) {
  ...
  // A coder worktree overlay preserves a case-colliding repo root prompt to
  // <workspace>/.advisor-root-prompt.md (see overlayAgentFiles). That file's
  // presence is the signal that a collision occurred AND preservation succeeded,
  // so the bootstrap prompt should point the worker at the preserved original.
  // The flag is a property of (agent, repo) — NOT per-worker — so two coders in
  // the same repo still share a byte-identical bootstrap prefix.
  const rootPromptCollision =
    agent === 'coder' &&
    fs.existsSync(path.join(workspace, '.advisor-root-prompt.md'));
