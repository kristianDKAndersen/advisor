...
// EVERY repo-tracked file the overlay would displace by a case-fold accident.
//
...
// (the case-fold IS the bug). This is NOT the root prompt alone: ANY tracked
// path the overlay writes under a DIFFERENT casing has the same failure.
//
// RULE — preserve on genuine case DIFFERENCE only. For each file the overlay is
// about to write (its path relative to agentSrc), if the repo tracks that same
// path folded to lowercase but under a DIFFERENT casing, copy the repo's
// checked-out version to <dest>/.advisor-preserved/<tracked-relative-path>
// (TRACKED casing, parent dirs created) BEFORE the overlay runs. If the repo
// tracks the path in the EXACT casing the overlay writes (e.g. tracked
// `.claude/settings.json` vs overlaid `.claude/settings.json`), the overlay
// replacing it is intended, pre-existing behaviour — a repo that legitimately
// ships its own file — and is deliberately NOT preserved (preserving it would
// change behaviour for every such repo). The decision is purely:
// trackedActual exists for the folded path AND trackedActual !== overlayPath.
//
...
function overlayAgentFiles(agentSrc, dest, repo) {
  ...
  for (const overlayPath of overlayRel) {
    ...
    // Fire ONLY on a genuine case difference — exact-casing matches are
    // intended, pre-existing overwrite behaviour (see RULE above).
    if (!trackedActual || trackedActual === overlayPath) continue;
    ...
    try {
      ...
      process.stderr.write(
        `[summon] overlay collision: repo tracks '${trackedActual}' under different ` +
          `casing than the agent's '${overlayPath}'; on a case-insensitive filesystem ` +
          `they are one file, so the overlay would destroy the repo's own version. ` +
