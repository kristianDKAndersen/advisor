...
function composeBootstrapPrompt({
  ...
  const chanCmd = `node "$ADV/lib/channel.js"`;
  const closeTab = `bash "$ADV/bin/close-tab"`;
...
3. Do the work. Write deliverables per the "where to write files" rules above —
   ...
   **Between every action step**, check for new inbox messages:

       ${chanCmd} recv --file "$INBOX" --after <last_seq> --json

   Update \`last_seq\` after each check. On \`terminate\`, immediately run \`${closeTab}\` and stop — do not send \`result\`.

   **If the task has no immediate work** (e.g. "stand by", "wait", "idle"): never sit idle.
   Tail the inbox in a blocking loop until \`terminate\` arrives:

       ${chanCmd} tail --file "$INBOX" --after <last_seq> --timeout 300 --json

   Re-tail on every timeout — loop: tail → check result → if terminate: close-tab, else re-tail.

4. When a deliverable is ready, send a \`result\` message that includes the full
   absolute path of every file you wrote.
5. After sending \`result\`, **self-terminate immediately**. Your FINAL tool call must be:

       ${closeTab}

   This closes your Terminal tab and ends your session. Do not tail the inbox or wait
   for follow-up. The Advisor spawns a fresh worker for any refinements.

6. On \`terminate\` (received **at any point** while alive): your FINAL tool
   call must be:

       ${closeTab}

   Do not send any more messages, do not continue work, do not ask why.
...
exec claude "$(cat ${shesc(promptFile)})"
`
  );
