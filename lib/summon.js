...
function composeBootstrapPrompt({
  ...
  const _prompt = `You are the **${agentName}** worker for advisor session \`${sid}\`.
...
` : ''}`;
  ...
  try {
    ...
    const { queryEpisodes } = require('./episodes');
    const task_hash = createHash('sha256').update((goal || '').slice(0, 200)).digest('hex');
    const eps = queryEpisodes(task_hash, 3);
    if (eps.length > 0) {
      const section = '\n\n## Past episodes\n' + eps.map(e =>
        '- **' + e.established + '**' +
...
function provisionOne(args, overrideSid) {
  const { agent, task, goal, model, intelligence, allowedTools, tui, timeoutSec, subTeam, subTeamModel, protectedTests } = args;
  ...
  // ADVISOR_WORKER_HOOKS=1 only for agents that opt in to trace/inbox-poll overhead.
  const WORKER_HOOKS_ALLOWLIST = new Set(['planner', 'researcher']);
  {
    ...
    settings.env.ADVISOR_WORKER_HOOKS = WORKER_HOOKS_ALLOWLIST.has(agent) ? '1' : '0';
  ...
  // Per-worker advisor-tool model: Fable workers advise with fable; every other
  // resolved model advises with opus (advisor must be same-or-higher tier). When
  // no model is resolved (no --intelligence, no --model), omit the flag so the
  // worker inherits the global advisorModel rather than risk a fail-fast mismatch.
  const advisorToken = effectiveModel
    ? (effectiveModel.includes('fable') ? 'fable' : 'opus')
    : null;
  const advisorFlag = advisorToken ? `--advisor ${advisorToken} ` : '';
  ...
  const claudeCore = `claude --permission-mode auto ${modelFlag}${advisorFlag}${allowedToolsFlag}--strict-mcp-config -- "$(cat ${shesc(promptFile)})"`;
...
module.exports = { composeBootstrapPrompt, trustWorkspaceInClaudeConfig, resolveIntelligence, provisionOne, buildProtectedTestsEnv, buildMcpDenylist, parseArgs, injectWorkerHooks };
