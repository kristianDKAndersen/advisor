...
const { resolveNextAgent } = require('./chain');
...
// Expand shell substitutions $(...) in skill content, mirroring lib/skill-expand.js.
...
function provisionOne(args, overrideSid) {
  const { agent, task, goal, model, intelligence, allowedTools, tui } = args;
  ...
  try {
    const prefResult = spawnSync('node', ['-e',
      "require('./lib/preflight').preflight({prompt:process.env._PF_PROMPT,timeoutMs:1500})" +
  ...
  try {
    ...
    function linkSkill(src, dest) {
      ...
      if (/\$\([^)]+\)/.test(content)) {
        ...
        fs.writeFileSync(path.join(dest, 'SKILL.md'), expandContent(content));
      } else {
  ...
  // Pattern 5.3: explicit --allowedTools wins; otherwise fall back to default_tools frontmatter.
  let effectiveAllowedTools = allowedTools;
  if (!effectiveAllowedTools) {
    const claudeMdPath = path.join(session.ADVISOR_ROOT, 'agents', agent, 'CLAUDE.md');
    const fm = agents.parseFrontmatter(claudeMdPath);
    if (Array.isArray(fm.default_tools) && fm.default_tools.length > 0) {
      effectiveAllowedTools = fm.default_tools.join(',');
    }
  }
  ...
  // TUI mode omits --print so claude opens an interactive session in the tab.
  // Headless mode keeps --print so claude runs non-interactively and exits.
  // Both modes keep the -- terminator (load-bearing: prevents --allowedTools
  // from consuming the prompt arg; see commit 76aac8c).
  const claudeCoreTui = `claude --permission-mode auto ${modelFlag}${allowedToolsFlag}-- "$(cat ${shesc(promptFile)})"`;
  const claudeCoreHeadless = `claude --print --permission-mode auto ${modelFlag}${allowedToolsFlag}-- "$(cat ${shesc(promptFile)})"`;
  // Headless: wrap in retry loop (max 3 attempts, sleeps 5/15s, final attempt uses exec).
  // TUI: exec directly (interactive sessions don't need transient-error retry).
  const claudeLaunchBody = tui
    ? `exec ${claudeCoreTui}`
    : `_attempt=0
while [ "$_attempt" -lt 2 ]; do
  _err=$(mktemp)
  set +e
  ${claudeCoreHeadless} 2>"$_err"
  _ec=$?
  set -e
  if [ "$_ec" -eq 0 ]; then rm -f "$_err"; exit 0; fi
  _cls=$(ADV_ERR_FILE="$_err" ADV_ERR_CODE="$_ec" node -e "const{classifyError}=require(process.env.ADV+'/lib/transient-retry');process.stdout.write(classifyError({stderr:require('fs').readFileSync(process.env.ADV_ERR_FILE,'utf8'),exitCode:Number(process.env.ADV_ERR_CODE)}))" 2>/dev/null || echo unknown)
  rm -f "$_err"
  [ "$_cls" = "transient" ] || exit "$_ec"
  case "$_attempt" in 0) sleep 5;; 1) sleep 15;; esac
  _attempt=$((_attempt + 1))
done
exec ${claudeCoreHeadless}`;
  fs.writeFileSync(
    launchScript,
    `#!/usr/bin/env bash
set -e
