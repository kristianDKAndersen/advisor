...
// Overlay the agent's files (spawns/<agent>/) onto a git worktree, preserving
// EVERY repo-tracked file the overlay would displace.
//
// The overlay is meant to land as UNTRACKED files. But on a case-INSENSITIVE
// filesystem (macOS default), an overlaid `CLAUDE.md` and a repo-tracked
// `claude.md` are the SAME inode, so a naive copy silently overwrites the
// tracked content. `git ls-files` prints paths case-SENSITIVELY, so detection
// MUST fold case — a literal `CLAUDE.md` lookup would miss tracked `claude.md`
// (the case-fold IS the bug). This is NOT the root prompt alone, and it is not
// only case-fold accidents either: ANY tracked path the overlay writes has the
// same failure, whether the casing differs (filesystem accident) or matches
// exactly (a repo that legitimately tracks its own `.claude/settings.json` at
// the same path the overlay writes) — both are the overlay silently destroying
// a file the repo owns.
//
// RULE — preserve on ANY collision, case-differing or exact. For each file the
// overlay is about to write (its path relative to agentSrc), if the repo
// tracks that same path folded to lowercase (under any casing), copy the
// repo's checked-out version to <dest>/.advisor-preserved/<tracked-relative-path>
// (TRACKED casing, parent dirs created) BEFORE the overlay runs. The overlay
// still wins at its own path afterward — preservation is purely additive, it
// never blocks or alters the overlay, it only makes the displaced original
// recoverable instead of gone.
//
// Cost, accepted deliberately: this now fires in ANY repo that tracks a file
// the overlay writes, not only case-fold accidents. CLAUDE.md is a common
// Claude Code convention, so many repos will now get an untracked
// .advisor-preserved/ directory in coder worktrees where they previously got
// none. Uniform beats two rules with a subtle boundary; the overlay's own
// behaviour is unchanged (it still always wins), only recoverability improves.
//
// The tracked-path LIST is read from `repo` (its index is always populated,
// which also sidesteps any index refresh a status on the fresh worktree might
// trigger); the preserved CONTENT is read from `dest` (the checked-out file).
// `.advisor-preserved` is never itself a collision target: the overlay walk
// only enumerates files under agentSrc (spawns/<agent>/), which never contains
// an `.advisor-preserved` entry, so no guard against clobbering it is needed.
//
// Returns { collision, preserved } where `preserved` is the list of
// { trackedPath, overlayPath, preservedPath } entries (empty on the
// no-collision path). With no collision the body is byte-for-byte the original
// localCopyDir(agentSrc, dest) and no `.advisor-preserved` directory is created
// — provably inert, which protects every coder run in every other repo.
function overlayAgentFiles(agentSrc, dest, repo) {
  let tracked = [];
  try {
    ...
    // Not a git repo / git unavailable: no collision possible — plain overlay.
  }
  // Case-folded index of tracked paths: lowercased path -> actual tracked casing.
  const trackedByLower = new Map();
  for (const t of tracked) trackedByLower.set(t.toLowerCase(), t);

  // Enumerate the repo-relative paths the overlay is about to write (every file
  // under agentSrc). git ls-files uses forward slashes, so build the same.
  const overlayRel = [];
  (function walk(dir, prefix) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      const rel = prefix ? `${prefix}/${entry.name}` : entry.name;
      if (entry.isDirectory()) walk(path.join(dir, entry.name), rel);
      else overlayRel.push(rel);
    }
  })(agentSrc, '');

  // Preserve BEFORE any overlay copy: run the full walk and all preservations
  // first, then a SINGLE localCopyDir. Preserving inline would risk saving
  // content the overlay had already clobbered when two overlay paths fold onto
  // one tracked path.
  const preserved = [];
  for (const overlayPath of overlayRel) {
    const trackedActual = trackedByLower.get(overlayPath.toLowerCase());
    // Fire on ANY collision — case-differing (filesystem accident) or exact
    // (repo legitimately tracks the same path the overlay writes). Either way
    // the overlay is about to destroy a file the repo owns; preserve it.
    if (!trackedActual) continue;
    const preservedPath = path.join(dest, '.advisor-preserved', trackedActual);
    try {
      fs.mkdirSync(path.dirname(preservedPath), { recursive: true });
      fs.copyFileSync(path.join(dest, trackedActual), preservedPath);
      preserved.push({ trackedPath: trackedActual, overlayPath, preservedPath });
      process.stderr.write(
        `[summon] overlay collision: repo tracks '${trackedActual}', which the ` +
          `agent overlay is about to write as '${overlayPath}'; the overlay would ` +
          `destroy the repo's own version. ` +
          `Preserved the original to '${preservedPath}' before overlaying.\n`
      );
    } catch (e) {
      // Preservation is load-bearing; if it fails, do NOT claim it was handled —
      // surface the failure and fall through (this file is not added to the list).
      process.stderr.write(
        `[summon] warn: could not preserve tracked file '${trackedActual}': ${e.message}\n`
      );
    }
  }

  // Overlay as before — byte-for-byte the original behaviour.
  localCopyDir(agentSrc, dest);
  return { collision: preserved.length > 0, preserved };
}
...
// Agents that run inside an isolated git worktree ($REPO === the worktree)
// instead of a copyDir template. EVERY isolation + teardown decision reads from
// this one set (here and in lib/channel.js), so adding an isolated agent later
// is a one-line change. NOT used for the coder-only timeout bump
// (scaledCoderTimeout stays gated on agent === 'coder').
const WORKTREE_AGENTS = new Set(['coder', 'doc-agent', 'planner']);
...
function provisionCoderWorktree(sid, agentName, repo) {
  ...
  // Overlay agent files on top of the worktree as untracked files. Guarded so a
  // case-fold collision with a repo-tracked root prompt (e.g. this repo's
  // lowercase `claude.md`) preserves the original instead of silently
  // destroying it — see overlayAgentFiles.
  overlayAgentFiles(agentSrc, dest, repo);
  return dest;
}
...
function composeBootstrapPrompt({
  ...
  let _ecoBlock = '';
  if (process.env.ADVISOR_ECO !== '0') {
    const { ECO_REVIEW_BLOCK, ECO_REVIEW_AGENTS, getEcoCoreBlock } = require('./eco-rules');
    _ecoBlock = (ECO_REVIEW_AGENTS.has(agentName) ? ECO_REVIEW_BLOCK : getEcoCoreBlock(agentName)) + '\n\n';
  }
...
${rootPromptCollision ? `
NOTE: the agent overlay had to displace one or more of this repo's own tracked
files on this case-insensitive filesystem — their names differ only by case from
files the overlay writes (e.g. this repo's \`claude.md\` vs your \`CLAUDE.md\`).
Each displaced original is preserved verbatim under
\`<worktree>/.advisor-preserved/<repo-relative-path>\` — read those when you need
the repo's own versions (the \`CLAUDE.md\` in your working directory is YOUR role
prompt, not the repo's).
` : ''}
${_ecoBlock}## Channel — how you talk to the Advisor
...
- \`$REPO\`        — your working copy of the target repo. For the isolated-worktree agents (\`coder\`, \`doc-agent\`, \`planner\`) this is a dedicated git worktree on its own branch \`ws/<sid>\`: your edits here do NOT land in the user's checkout and are torn down after the session, so any deliverable you want to keep must ALSO be copied to \`$OUTPUT_DIR/deliverables/\` (preserve repo-relative paths). ALL in-place edits go under \`$REPO\` and nowhere else.
...
// Reads a prior worker's persisted synthesis state for --continues wiring.
// next_action comes from session.json (set on every `synthesize` call);
// gap/key_quotes come from the last line of that session's synthesis.log;
// verdict comes from the last `result` message in that session's outbox.
// Fails open (returns null) if the prior sid has no readable state.
function readPredecessorState(priorSid) {
  try {
    const priorState = session.readSessionState(priorSid);
    if (!priorState) return null;
    const next_action = priorState.next_action || '';
    let gap = '', key_quotes = '';
    const synthLogFile = path.join(session.sessionDir(priorSid), 'synthesis.log');
    if (fs.existsSync(synthLogFile)) {
      const lines = fs.readFileSync(synthLogFile, 'utf8').trim().split('\n').filter(Boolean);
      if (lines.length > 0) {
        const last = JSON.parse(lines[lines.length - 1]);
        gap = last.gap || '';
        key_quotes = last.key_quotes || '';
      }
    }
    let verdict = '';
    const outboxFile = session.outboxPath(priorSid);
    if (fs.existsSync(outboxFile)) {
      const lines = fs.readFileSync(outboxFile, 'utf8').trim().split('\n').filter(Boolean);
      for (let i = lines.length - 1; i >= 0; i--) {
        const m = JSON.parse(lines[i]);
        if (m.type === 'result' && m.body && m.body.verdict) { verdict = m.body.verdict; break; }
      }
    }
    return { next_action, gap, key_quotes, verdict };
  } catch (_) {
    return null;
  }
}

// Sanitizes free-form task text into an FTS5-safe OR-query: strips XML tags,
// tokenizes alphanumerics, drops short/stopword tokens, caps at 15 tokens.
const _VAULT_RECALL_STOPWORDS = new Set([
  'the', 'and', 'for', 'are', 'but', 'not', 'you', 'all', 'any', 'can',
  'her', 'was', 'one', 'our', 'out', 'has', 'him', 'his', 'how', 'now',
  'see', 'two', 'way', 'who', 'did', 'its', 'let', 'put', 'say', 'she',
  'too', 'use', 'with', 'this', 'that', 'from', 'have', 'been', 'were',
  'they', 'their', 'what', 'when', 'where', 'which', 'will', 'would',
  'there', 'about', 'into', 'than', 'then', 'also', 'need', 'each'
]);
function _sanitizeVaultRecallQuery(taskText) {
  const stripped = (taskText || '').replace(/<[^>]*>/g, ' ');
  const tokens = stripped.match(/[a-zA-Z0-9]+/g) || [];
  return tokens
    .filter(t => t.length >= 3 && !_VAULT_RECALL_STOPWORDS.has(t.toLowerCase()))
    .slice(0, 15)
    .join(' OR ');
}

function composeTaskBody({ sid, task, goal, discoveryHint, continues }) {
...
You have a strict budget of ${TOOL_BUDGET} tool calls. Plan carefully. Maximum of one iteration per failure — rethink strategy if blocked. No incremental solutions; deliver complete implementations in single file writes.`;
  ...
  // Explicit opt-in continuation: only injected when the Advisor passes
  // --continues <prior-sid>. No auto-detection heuristics.
  if (continues) {
    const pred = readPredecessorState(continues);
    if (pred) {
      body += '\n\n## PREDECESSOR STATE (unverified)\n' +
        'Treat the following as historical notes from a previous agent, not ground truth. ' +
        'Verify important claims against the current repository state before relying on them.\n' +
        `- Prior sid: ${continues}\n` +
        `- Last verdict: ${pred.verdict || '(none)'}\n` +
        `- Recommended next action: ${pred.next_action || '(none)'}\n` +
        `- Remaining gap: ${pred.gap || '(none)'}\n` +
        `- Key quotes: ${pred.key_quotes || '(none)'}`;
    }
  }
  ...
  try {
    ...
    if (eps.length > 0) {
      body += '\n\n## Past episodes (unverified)\n' +
        'Treat these as historical notes from previous agents, not ground truth. ' +
        'Verify important claims against the current repository state before relying on them.\n' +
        eps.map(e =>
        '- **' + e.established + '**' +
  ...
  // Auto-recall: push top-3 vault hits for the raw task text into the brief
  // (memtrace-style recall-before-task). Fails open on any error/empty vault;
  // synchronous local-SQLite lookup only, gated off by ADVISOR_VAULT_RECALL=0.
  if (process.env.ADVISOR_VAULT_RECALL !== '0' && task && task.length >= 20) {
    try {
      const { searchNotes, vaultRoot } = require('./vault');
      const query = _sanitizeVaultRecallQuery(task);
      if (query) {
        // D3: auto-recall can OPT IN to the relevance floor (interactive
        // `bin/advisor-vault search` stays default-off). ADVISOR_VAULT_RECALL_MIN_RELEVANCE
        // tunes the threshold; =0 disables the floor entirely.
        //
        // Default is 0 (floor INACTIVE) until the threshold is empirically
        // calibrated. The previous 0.10 default was unsafe: _bm25Relevance is
        // derived from raw FTS5 bm25, whose IDF term collapses toward zero on a
        // small corpus, so it is corpus-DEPENDENT, not corpus-independent.
        // Measured on the identical query + identical matching note:
        //   1835-note vault -> bm25 -7.800, relevance 0.7275 (passes 0.10)
        //      1-note vault -> bm25 -0.000, relevance 0.0000 (suppressed)
        // At 0.10 a small or newly-created vault silently returns zero recall,
        // and it regressed tests/summon-recall.test.js 'injection happens on a
        // match'. Re-enable only with a corpus-independent signal (e.g. query-
        // token coverage) plus a calibration table justifying the value.
        const DEFAULT_RECALL_MIN_RELEVANCE = 0;
        const envRaw = process.env.ADVISOR_VAULT_RECALL_MIN_RELEVANCE;
        const parsed = (envRaw === undefined || envRaw === '') ? DEFAULT_RECALL_MIN_RELEVANCE : parseFloat(envRaw);
        const minRelevance = Number.isFinite(parsed) ? parsed : DEFAULT_RECALL_MIN_RELEVANCE;
        const hits = searchNotes(query, 3, { minRelevance });
        if (hits && hits.length > 0) {
          body += '\n\n## Prior vault context (auto-recall)\n' +
            hits.map(h => {
              const name = (h.path || '').replace(/^.*\//, '').replace(/\.md$/, '');
              const summary = (h.snippet || '').replace(/<\/?b>/g, '');
              const absPath = h.path ? path.join(vaultRoot(), h.path) : '';
              return `- **${name}** (${h.type || 'note'}): ${summary} — ${absPath}`;
            }).join('\n');
        }
      }
    } catch (_) {}
...
function injectWorkerHooks(existingHooks) {
  ...
  return {
    ...
    // and append this worker session's token usage to token-usage.jsonl so
    // bin/advisor-cost can resolve it (workers otherwise never run this hook —
    // it's only wired into the advisor's own top-level .claude/settings.json).
    Stop: [
      ...(existing.Stop || []),
      { matcher: '', hooks: [{ type: 'command', command: 'node $ADV/lib/hooks/worker-result-check.js' }] },
      { matcher: '', hooks: [{ type: 'command', command: 'node $ADV/.claude/hooks/stop-telemetry.js' }] },
    ],
  };
}
...
function provisionOne(args, overrideSid) {
  const { agent, task, goal, model, intelligence, allowedTools, tui, timeoutSec, subTeam, subTeamModel, protectedTests, toolBudget, continues } = args;
  ...
  // Provision workspace: git worktree for WORKTREE_AGENTS, copyDir for all others.
  const workspace =
    WORKTREE_AGENTS.has(agent)
      ? provisionCoderWorktree(sid, agent, repo)
      : session.provisionWorkspace(sid, agent);

  // node_modules symlink: give worktree workers access to repo deps without running
  // 'bun install'. Fail-open — if the repo has no node_modules, skip silently.
  if (WORKTREE_AGENTS.has(agent)) {
  ...
  // SAFETY: a WORKTREE_AGENTS worker runs in an isolated git worktree
  // (provisionCoderWorktree), so its $REPO must point at that worktree — NEVER the
  // original repo. Otherwise, in self-invocation (repo === ADVISOR_ROOT) it would
  // edit the live main tree in place via $REPO/... absolute paths, defeating
  // worktree isolation and risking catastrophic edits to the running advisor. For
  // all other agents $REPO stays the original repo (they get a copyDir template,
  // not a repo checkout).
  const workerRepo = WORKTREE_AGENTS.has(agent) ? workspace : repo;
  ...
  // Write an initial session.json at provision time (only if one doesn't
  // already exist) so the reaper has a real staleness signal from the
  // start — previously session.json was only written at synthesize time, so
  // a freshly-summoned session had none and could be mistaken for stale by
  // reaperSweepOrphanSessions. Non-destructive: callers that pre-seed
  // session.json (e.g. tier-conditional provisioning) must not be clobbered.
  if (!session.readSessionState(sid)) {
    session.writeSessionState(sid, { ...session.defaultSessionState(sid), status: 'in-flight' });
  }
  ...
  channel.append(inbox, {
    type: 'task',
    body: composeTaskBody({ sid, task, goal, discoveryHint: isVague, continues }),
    from: 'advisor'
  });

  // A coder worktree overlay preserves every case-colliding repo file into
  // <workspace>/.advisor-preserved/ (see overlayAgentFiles). That directory's
  // presence is the signal that at least one collision occurred AND preservation
  // succeeded, so the bootstrap prompt should point the worker at the preserved
  // originals. The flag is a property of (agent, repo) — NOT per-worker — so two
  // coders in the same repo still share a byte-identical bootstrap prefix.
  const rootPromptCollision =
    WORKTREE_AGENTS.has(agent) &&
    fs.existsSync(path.join(workspace, '.advisor-preserved'));
  ...
  const effortFlag = resolvedReasoning ? `--effort ${shesc(resolvedReasoning)} ` : '';
  ...
  // Durable stderr capture: appended across every retry attempt, never deleted
  // on exit (unlike the per-attempt mktemp file, which is scratch for
  // classifyError only). Lets the advisor read WHY a worker died without
  // opening the tmux pane.
  const launchStderrLog = path.join(session.sessionDir(sid), 'launch-stderr.log');
  ...
  // the script (|| _exit_code=$? is not subject to set -e). ensure_result is
  // armed via `trap ... EXIT` (not a plain last line) so it ALWAYS fires no
  // matter how the script exits — including a `set -e` abort from an
  // unprotected command-substitution assignment (e.g. `_ce_tmp=$(mktemp)` or
  // `_ce_class=$(classifyError ...)` failing) that would otherwise skip past
  // a final "last line" call and leave the outbox silently empty. It is a
  // no-op when the worker already wrote its own result envelope.
  const claudeCore = `claude --permission-mode auto ${modelFlag}${effortFlag}${allowedToolsFlag}${disallowedToolsFlag}--strict-mcp-config -- "$(cat ${shesc(promptFile)})"`;

  const claudeLaunchBody = [
    `_launch_log=${shesc(launchStderrLog)}`,
    '_exit_code=0',
    '_attempt=0',
    '_max_attempts=3',
    'classifyError() {',
    ...
    '}',
    'ensure_result() {',
    '  _stderr_tail=$(tail -n 20 "$_launch_log" 2>/dev/null || true)',
    '  bun "$ADV/lib/channel.js" ensure-result --file "$OUTBOX" --exit-code "${_exit_code:-unknown}" --reason "$_stderr_tail" --from wrapper --quiet || true',
    '}',
    'trap ensure_result EXIT',
    'while [ $_attempt -lt $_max_attempts ]; do',
    '  _attempt=$(( _attempt + 1 ))',
    '  _ce_tmp=$(mktemp)',
    `  ${claudeCore} 2>"$_ce_tmp" || _exit_code=$?`,
    '  cat "$_ce_tmp" >> "$_launch_log" 2>/dev/null || true',
    '  if [ $_exit_code -eq 0 ]; then rm -f "$_ce_tmp" 2>/dev/null || true; break; fi',
...
module.exports = { composeBootstrapPrompt, composeTaskBody, trustWorkspaceInClaudeConfig, resolveIntelligence, provisionOne, buildProtectedTestsEnv, buildMcpDenylist, parseArgs, injectWorkerHooks, scaledCoderTimeout, overlayAgentFiles, WORKTREE_AGENTS };
