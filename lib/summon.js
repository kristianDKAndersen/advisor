...
- \`$REPO\`        — the user's repo root (for self-invocation: = ADVISOR_ROOT)
...
function injectWorkerHooks(existingHooks) {
  ...
  return {
    ...
    PostToolUse: [
      ...
      { matcher: 'Bash', hooks: [{ type: 'command', command: 'node $ADV/lib/hooks/worker-output-filter.js' }] },
...
function provisionOne(args, overrideSid) {
  ...
  const WORKER_HOOKS_ALLOWLIST = new Set(['planner', 'researcher', 'doc-agent']);
  // ADVISOR_OUTPUT_FILTER=1 enables per-call Bash output filtering for build/test-heavy workers.
  // PROTOTYPE — gated OFF for ALL agents pending (a) empirical confirmation the PostToolUse
  // updatedToolOutput contract works end-to-end, and (b) fix of the assertion-diff scoring
  // defect (Expected:/Received: lines score 0.1 and get dropped; see MEASUREMENT-output-filter.md).
  // Re-populate this set only after both gates are closed.
  const OUTPUT_FILTER_ALLOWLIST = new Set([]);
  {
    ...
    settings.env.ADVISOR_OUTPUT_FILTER = OUTPUT_FILTER_ALLOWLIST.has(agent) ? '1' : '0';
...
module.exports = { composeBootstrapPrompt, composeTaskBody, trustWorkspaceInClaudeConfig, resolveIntelligence, provisionOne, buildProtectedTestsEnv, buildMcpDenylist, parseArgs, injectWorkerHooks };
