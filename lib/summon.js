...
function provisionOne(args, overrideSid) {
  const { agent, task, goal, model, intelligence, allowedTools, tui, timeoutSec } = args;
  ...
  const claudeCoreHeadless = `claude --permission-mode auto ${modelFlag}${allowedToolsFlag}-- "$(cat ${shesc(promptFile)})"`;
  // Headless: exec directly; retry is owned by tmux-runner.js at the outer level.
  // TUI: exec directly (interactive sessions don't need retry).
