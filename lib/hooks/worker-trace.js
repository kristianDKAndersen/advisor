...
async function main() {
  ...
  // result_summary: first 80 chars of output (CC 2.1.177+: stdout/stderr; legacy: output)
  let resultSummary = '';
  if (typeof response.stdout === 'string') {
    const combined = response.stderr ? response.stdout + response.stderr : response.stdout;
    resultSummary = combined.slice(0, 80).replace(/\n/g, ' ');
  } else if (typeof response.output === 'string') {
    resultSummary = response.output.slice(0, 80).replace(/\n/g, ' ');
  } else if (response.error) {
    resultSummary = 'error: ' + String(response.error).slice(0, 72);
  } else {
    resultSummary = JSON.stringify(response).slice(0, 80);
  }
