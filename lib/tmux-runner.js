  ...
  return withTuiLock(lockDir, () => {
    ...
    if (!windows.includes('tui')) {
      ...
      const placeholderPaneId = exec('tmux', [
        ...
        '--', 'bash', '-c', "stty -echo -icanon 2>/dev/null; echo '[advisor] holding tui window open - workers attach as new panes (do not type here)'; while :; do sleep 86400; done",
      ]).trim();
    ...
    // Move tmux focus to the worker pane so the placeholder never holds keyboard focus.
...
/**
 * Poll for a Stop-hook sentinel file. When `ownerDir` is given, validates
 * that the sentinel's JSON payload (written by the Stop hook) actually
 * belongs to this worker before accepting it: any `claude` child process
 * spawned by a worker's Bash tool inherits the same CLAUDE_I_SENTINEL env
 * var, so its own Stop hook can write to THIS worker's sentinel path when
 * the child's turn ends. Without validation that cross-session write is
 * mistaken for the worker's own completion and the wrapper kill-panes a
 * still-running worker (observed exit-137 on in-flight calls, 2026-07-07).
 * On a mismatch, both sentinel files are deleted and polling continues —
 * it is never treated as a timeout/failure of the real worker.
 *
 * @param {string} sentinelPath - path to the <sentinel>.done marker file.
 * @param {number} timeoutMs
 * @param {number} [intervalMs=300]
 * @param {string|null} [ownerDir] - expected workspace/project dir (payload.cwd
 *   must equal this or be nested under it). Omit to skip ownership validation.
 */
async function pollSentinel(sentinelPath, timeoutMs, intervalMs = 300, ownerDir = null) {
  const sentinelJsonPath = sentinelPath + '.json';
  ...
  while (Date.now() < deadline) {
    if (fs.existsSync(sentinelPath)) {
      if (ownerDir) {
        let payload = null;
        try {
          payload = JSON.parse(fs.readFileSync(sentinelJsonPath, 'utf8'));
        } catch (_) {
          // Missing/malformed payload — not an ownership mismatch, let the
          // caller's existing parse-failure handling deal with it.
          payload = null;
        }
        if (payload && Object.prototype.hasOwnProperty.call(payload, 'cwd')) {
          const payloadCwd = String(payload.cwd || '');
          const owned = payloadCwd === ownerDir || payloadCwd.startsWith(ownerDir + path.sep);
          if (!owned) {
            try { fs.unlinkSync(sentinelPath); } catch (_) {}
            try { fs.unlinkSync(sentinelJsonPath); } catch (_) {}
            process.stderr.write(
              `[tmux-runner] pollSentinel: discarded foreign sentinel payload (cwd=${payloadCwd}, expected under ${ownerDir}); continuing to poll\n`
            );
            const wait = Math.min(intervalMs, Math.max(0, deadline - Date.now()));
            if (wait > 0) await sleep(wait);
            continue;
...
function reaperSweepOrphanSessions(opts = {}) {
  ...
  for (const sessionName of sessions) {
    ...
    // Grace floor: never reap a session younger than 2h, regardless of
    // session.json state — a freshly-summoned session has no session.json
    // yet (only written at synthesize time), so isStale would otherwise be
    // true and a live just-launched worker could be killed if pgrep -f <sid>
    // happens to miss (claude argv rarely contains the sid). Mirrors
    // reapStaleWorktrees' grace-floor pattern. The sid's leading epoch
    // (e.g. 1783413687-63e89e) gives the session age; sids without that
    // prefix get no grace (age unknown).
    const graceTsMatch = suffix.match(/^(\d{9,})-/);
    if (graceTsMatch && now - Number(graceTsMatch[1]) * 1000 < 2 * 3600 * 1000) continue;
  ...
  if (process.env.ADVISOR_TMUX_MULTIPLEX === '1') {
    ...
    for (const windowName of advisorWindows) {
      ...
      // Grace floor: never reap a window whose sid is younger than 2h
      // (same rationale as the advisor-<sid> session sweep above).
      const winGraceTsMatch = matchingEntry.match(/^(\d{9,})-/);
      if (winGraceTsMatch && now - Number(winGraceTsMatch[1]) * 1000 < 2 * 3600 * 1000) continue;
...
async function spawnHeadless({
  ...
  try {
    ...
    const done = await pollSentinel(sentinel, timeoutMs, 300, path.dirname(launchScript));
    if (!done) {
