 ...
 * Format: "<agent>-<sid[:8]>" or "worker-<sid[:8]>"
 */
function makeWindowName(agent, sid) {
  const short = sid.slice(0, 8);
  return agent ? `${agent}-${short}` : `worker-${short}`;
}

/**
 * Idempotently ensure the shared 'advisor' tmux session exists.
 * Uses new-session -A -d so concurrent callers are race-safe.
 *
...
function ensureAdvisorSession(execFn) {
  ...
  fn('tmux', ['new-session', '-A', '-d', '-s', 'advisor', '-x', '220', '-y', '50']);
}
...
function reaperSweepOrphanSessions(opts = {}) {
  ...
  if (process.env.ADVISOR_TMUX_MULTIPLEX === '1') {
    ...
    for (const windowName of advisorWindows) {
      // Window name format: <agent>-<short8> or worker-<short8>
      const parts = windowName.split('-');
      if (parts.length < 2) continue;
      const shortSid = parts[parts.length - 1];
      if (shortSid.length !== 8) continue;

      const matchingEntry = runDirEntries.find(entry => entry.slice(0, 8) === shortSid);
      let isStale = true;
      if (matchingEntry) {
      ...
      let hasLive = false;
      try { execFn('pgrep', ['-f', shortSid]); hasLive = true; } catch (_) {}
...
async function spawnHeadless({
  ...
  try {
    ...
    if (preCreatedPaneId) {
      ...
      // Kill stale window-0 from new-session (idempotent)
      try { exec('tmux', ['kill-window', '-t', 'advisor:0']); } catch (_) {}
      exec('tmux', ['pipe-pane', '-t', paneId, `cat >> ${shquote(logFile)}`]);
    }
