...
function ensureAdvisorSession(execFn) {
  ...
  try {
    fn('tmux', ['new-session', '-A', '-d', '-s', 'advisor', '-x', '220', '-y', '50', '-n', '__advisor_scratch__']);
  } catch (_) {
...
function reaperSweepOrphanSessions(opts = {}) {
  ...
  if (process.env.ADVISOR_TMUX_MULTIPLEX === '1') {
    ...
    for (const windowName of advisorWindows) {
      ...
      // Match by checking which run dir entry appears as a suffix of the window name.
      ...
      let hasLive = false;
      const pgrepPattern = matchingEntry || windowName;
      try { execFn('pgrep', ['-f', pgrepPattern]); hasLive = true; } catch (_) {}
