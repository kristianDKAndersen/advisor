...
// too (some in-process callers may pass one directly). Returns undefined to
// signal "could not be parsed into a usable object" (malformed JSON, or JSON
// that parses to a non-object primitive like a bare string/number) so the
// caller can tell that apart from a genuinely empty-but-valid object body -
// the two must not be treated the same way (see classifySessionDir).
...
function classifySessionDir(sessionDir) {
  ...
  if (lastResult) {
    ...
    if (body === undefined) {
      // Unparseable (malformed JSON) or parsed to a non-object primitive:
      // genuinely indistinguishable from a real failure (a wrapper dying
      // mid-write truncates its final line the same way), so this must NOT
      // fall through to clean-result. Bucket under pane-death/deterministic
      // per the module's ambiguous-defaults-to-deterministic rule.
      return verdict('pane-death', false, `channel/outbox.jsonl: result body unparseable/non-object (from=${lastResult.from}), defaulting deterministic`);
    }
